...
description: 涉及用户当前打开文件 / 选中状态 / 屏幕实时数据时，先查 skills/ 和 acceptance/，不要凭默认能力边界否认
type: feedback
originSessionId: 7de9f536-08a2-4eb4-b494-0d7d0c3ee6e6
---
# Check `skills/` and `acceptance/` before saying "I can't"

任何涉及"用户当前打开的文件 / 选中的对象 / 屏幕实时状态"的请求，**第一反射是 `Glob skills/* acceptance/*`**，不是先用默认 Claude 能力边界拒绝。

> 项目结构注：2026-05-27 起 debug/ 改名为 【Mc-debug】/，只放 .md 手工记录；可执行 probe 脚本（probe_*.py / save_smoke_*.py / export_active_*.py）改放 `acceptance/`。
...
- 第一步：`Glob skills/read_*`、`Glob acceptance/probe_*`、`Grep "选中\|active\|GetActiveObject"`
- 第二步：找到对应脚本就直接 `python skills/xxx.py` 跑，输出贴回对话
- 第三步：找不到才说"项目里没有现成的，要我写一个吗？"
- **决不**：凭"我没有访问 X 的能力"这种通用边界回答先否认
- 涉及 PPT/Excel COM 时，本项目的工具集 = `skills/*.py`、`acceptance/*.py`、`src/Function_030.py` 里的 helper、`src/_ppt_shared.py`，扫一遍再下判断
