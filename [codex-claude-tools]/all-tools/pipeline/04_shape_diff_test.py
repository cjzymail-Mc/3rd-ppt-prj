#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Step 4: Terminal verification — three-layer diff test.

Role: Final QA sign-off. 03b now handles self-check + auto-fix internally;
this step serves as an independent terminal verification that produces a
report but does NOT drive fix loops. If 03b's self-check passed, this step
should confirm the result. Any issues found here indicate self-check gaps.

Compares Template page vs claude-ppt.pptx page 1.
...
Also reads 03b-self_check_report.md (if exists) for cross-reference.
"""
...
def main() -> int:
    ...
    try:
        ...
        # Cross-reference 03b self-check report
        sc_report = PROGRESS_DIR / "03b-self_check_report.md"
        if sc_report.exists():
            safe_print(f"[INFO] 03b self-check report found: {sc_report.name}")
        else:
            safe_print("[INFO] No 03b self-check report (03b may predate self-check feature)")
