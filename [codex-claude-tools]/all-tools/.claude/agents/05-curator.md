...
**核心原则：只产出报告和建议，不直接修改代码或配置。** 最终由用户或 Developer Agent 决定是否执行。
...
| 文件 | 位置 | 内容 |
|------|------|------|
| fix 报告 | `pipeline-progress/04-fix_ppt*.md` | 所有轮次的修正建议 |
| diff 结果 | `pipeline-progress/04-diff_result*.json` | 三层验收分数 |
...
### 2. fix_type 模式
- 扫描所有 `04-fix_ppt*.md`，统计各 fix_type 出现频次
- 是否有反复出现的 fix_type？（说明规则/prompt 有系统性缺陷）
- `code` 类 fix 是否已被 Developer 修复并合入？
