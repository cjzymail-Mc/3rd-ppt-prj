...
然后，将这些新增元素的信息，汇总成shape-detail.md（假设你识别出一共有9个新增元素），详细列出上面所有信息，并提供给我
...
构建 shapes 内容：
    ...
    你帮我写一个py测试脚本（02-build_ppt_com.py），主要包含9个函数（假设你识别出一共有9个新增shape元素）
    这9个函数的主要功能，是从【源数据-问卷sheet】中提取对应的信息，加工成对应9个shape的精确内容
    你可以通过对比分析【标准ppt模板（Template 2.1.pptx 第15页）】和【2025 数据 v2.2.xlsx - 问卷sheet】，来构建这些函数
    （标准ppt模板的内容，完全源于 问卷sheet）
    注意，我原来脚本中包含【GPT-5通讯函数】，可以用来连接gpt服务器，实现问卷信息的总结和提取，你仅需构建对应的 prompt 即可
    你可以在新的脚本中（02-build_ppt_com.py）重复利用（import）这些现成的函数，不需要重写。


构建ppt文件：
    另外，完成9个shape内容的构建后，
    你的这个（02-build_ppt_com.py）这个脚本，
...
------第3步----- 
你完成ppt文件生成后，需要再写一个测试脚本（03-shape_diff_test.py），
使用 COM 打开【标准模板（位于Template 2.1.pptx第15页）】和【codex.pptx（第1页，你生成的ppt应该只有1页）】，
逐 shape 对比位置、大小、字体、颜色、图表类型等关键属性，
...
直至你生成的最新ppt文件能够通过（03-shape_diff_test.py）的验证（视觉保真度必须达到 98%+（几乎和手动复制 slide 后改数据一样）
