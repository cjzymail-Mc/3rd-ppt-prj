...
│         ↓                prompt_trace.json             │
│  03_build_ppt_com.py ──→ claude-ppt X.Y.pptx              │
...
├── claude-ppt X.Y.pptx                     # Step 3B 产出（最终PPT）
...
**输出**：`claude-ppt {version}.pptx`, `build-ppt-report.md`, `post_write_readback.json`

### `04_shape_diff_test.py` — Step 4: 三层差异测试

**对比对象**：Template 第15页 vs claude-ppt.pptx 第1页
...
python pipeline/04_shape_diff_test.py --target "claude-ppt 1.0.pptx"  # Step 4
```
