...
🚨 **重要：文件路径规范**
- 所有输出文件（PLAN.md、PROGRESS.md、BUG_REPORT.md、SECURITY_AUDIT.md 等）必须保存在**项目根目录**
- 使用 Write 工具时，直接使用相对路径（如 `PLAN.md`，不要加任何目录前缀）
- 源代码文件按 PLAN.md 指定位置（如 `src/main.py`）
- 始终使用正斜杠 `/` 作为路径分隔符
"""
...
1. **第一步：代码库分析**
   ...
   - **使用 Write 工具**生成 `CODEBASE_ANALYSIS.md`（保存到项目根目录），包含：
     * 项目结构（目录树 + 核心模块说明）
     * 技术栈（语言、框架、库）
     * 代码风格和设计模式
     * 关键文件清单
