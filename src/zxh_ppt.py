...
# ---------------------------------------------------------------------------
# Hardcoded shape specs (zxh 标准模板在 Template 2.1.pptx 第 17 页)
# 仅 2 个文本框：TextBox 15（综合总结）+ TextBox 17（修改建议）
# 两个 shape 都使用 section-aware 染色（同一 shape 内可同时含优势/问题段）
# ---------------------------------------------------------------------------
ZXH_SHAPES = [
    {
        "name": "TextBox 15",
...
# zxh 标准模板所在页（Template 2.1.pptx 第 17 页）
_TEMPLATE_SLIDE = 17
...
# 定制说明：   zxh 独有 p1p2 模式（TextBox 17 的结构化修改建议），以及 focus=""
#             时的「优势/问题」两段式输出——这些差异在 pipeline 版本更新时
#             需要保留。如需同步最新改动，请手动 diff gpt_summary.md 并
#             更新 synced_at。
def _build_rich_prompt(
    ...
    fmt='p1p2' → structured P1/P2 modification action items.
    focus (non-empty, no fmt) → free-form summarization on that topic.
    focus='' → two-section 优势/问题 summary.
    """
    ...
    if fmt == "p1p2":
        ...
        task_line = (
            ...
            f"请帮我归纳为「优势」和「问题」两段。\n"
            f"先写「优势」段（正面体验），再写「问题」段（改进点）。\n"
            f"在每条结论后注明（X/{n}）表示几分之几的测试者有此反馈。\n"
            f"每段结论中，请将最核心的1-2个关键性能词用【】括起来（仅括词本身，不含标点），"
            f"例如：【止滑性】在室内场地表现稳定（2/{n}）。这些关键词后续会自动高亮显示。"
        )
        format_note = "- 严格按照「优势 / 问题」两段式的结构输出\n"
...
# ---------------------------------------------------------------------------
# Image extraction (kept for extensibility — zxh 当前模板未使用图片)
# ---------------------------------------------------------------------------
...
def _build_content(spec: dict, rows: List[List[Any]],
    ...
    if strategy == "extract_image":
        return ""  # handled separately in make_zxh_slide
...
def make_zxh_slide(mc_sht, mc_ppt, mc_slide, sample_name: str,
    ...
    """Generate zxh evaluation slide, integrated into Main.py Copy->Paste flow.

    Clones Slide(17) (zxh 标准模板页) to end of presentation,
    then writes all shape content per ZXH_SHAPES specs.
    ...
    for spec in ZXH_SHAPES:
        ...
        if strategy == "mean_extraction" or bool(_com_get(shp, "HasChart", False)):
            ...
            if ok and strategy == "gpt_prompted":
                ...
                _color_section_headers(shp)   # Fix 5: 章节标题红/蓝染色
