...
class Text_Grade(Text_Box):
    '评级文字（例如 A），大号 Arial 粗体'

    def __init__(self, Slide, Left, Top, Text, scale=1):
        Text_Box.__init__(self, Slide)

        self.tr.Text = Text
        self.tr.Font.Name = "Arial"
        self.tr.Font.Size = 72
        self.tr.Font.Bold = 1 # True
        self.tr.Font.Color.RGB = 0 # Black

        self.shape.Fill.Transparency = 1 # No fill
        self.shape.Line.Visible = 0 # No line

        self.shape.Height = 122
        self.shape.Width = 43 * scale # Approximate width based on "A"

        self.shape.Left = Left
        self.shape.Top = Top

class Text_Score(Text_Box):
    '评分文字（例如 8.29/10），微软雅黑 粗体'

    def __init__(self, Slide, Left, Top, Text, scale=1):
        Text_Box.__init__(self, Slide)

        self.tr.Text = Text
        self.tr.Font.NameFarEast = "微软雅黑"
        self.tr.Font.Name = "Arial" # Fallback/Mixed
        # Size was not detected, but looks large. Let's guess or leave default to be set manually if needed.
        # Based on "Grade" is 72, Score is smaller. Maybe 24-36?
        # But wait, I am adding this class to satisfy the requirement.
        # In my main script, I will mostly clone.

        self.tr.Font.Bold = 1
        self.tr.Font.Color.RGB = 128 # Dark Red? The JSON said "C00000" which is Hex.
        # C00000 is 12582912 in Decimal (R=192, G=0, B=0).
        # Win32Com RGB is usually integer. Red is 255 (0000FF), but here it's R at high byte?
        # No, Office Color is usually B G R (Little Endian).
        # C00000 (Hex RGB) -> R=C0, G=00, B=00.
        # Little Endian: 00 00 C0.
        # So 0xC0 = 192.
        self.tr.Font.Color.RGB = 192

        self.shape.Fill.Transparency = 1
        self.shape.Line.Visible = 0

        self.shape.Height = 36
        self.shape.Width = 155 * scale
